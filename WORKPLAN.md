...
| **F** Admin + Worker + SEO | `agent/f-admin-worker-infra` | 2 | `src/app/admin/`, `src/components/admin/`, `src/server/jobs/`, `src/middleware.ts`, `src/app/health/`, `src/app/ready/`, `src/app/sitemap.ts`, `src/app/robots.ts`, `src/app/api/internal/` | A | ⬜ chờ |
| **G** Tests | `agent/g-tests` | 3 | `tests/`, `playwright.config.ts`, `e2e/` | A–F merged | 🟢 xong |
...
- [NOTE] (G) — 2026-09-02 — Gói G hoàn thành trên nhánh `agent/g-tests` (base main sau A+B+E merge; C/D/F chưa merge). Unit (7 files, 44 test): format tz/DST (EDT/EST/ICT), slugify diacritics, rate-limit sliding window (block + window slide + isolation), pagination meta, standings computation invariants, sanitize HTML whitelist, slug determinism. Integration (6 files, 24 test) trên DB riêng `sport_g_test` (global-setup drop+create+migrate+seed): auth flow (token issue→consume qua dev-log capture, verify/reset, single-use, sha256-only persist, password round-trip), favorites CRUD + IDOR isolation, comments (insert/thread/pending queue), matches cache TTL (spy repo: không gọi DB 2 lần trong TTL, invalidate re-query), search trigram ("man utd"→Man United, typo "unitede", ranking), SSE route thật (retry/connected frames, hub publish→stream, 400 invalid topics). E2E Playwright (9 specs, desktop 1280px + mobile 375px chromium): 23 pass, 7 skip (feature-detect C/D/F chưa merge — match tabs, team page, admin), webServer tự boot dev :3100 với DB `sport_g_e2e`. Coverage: `npm run test:coverage` (v8 text+html; format 94%, rate-limit 100%, auth email 96%, cache 72%, matches service 72%, search repo 81%).
- [BLOCKER→BUG] (G) — 2026-09-02 — Phát hiện 2 bug ở gói khác (không sửa, theo mục 0.3): (1) **B login form hỏng**: `src/app/(auth)/login/page.tsx` post `{email, password}` nhưng API A nhận `{identifier, password}` → mọi UI login fail "Dữ liệu không hợp lệ" (E2E `test.fail` có chủ đích bắt bug này — sẽ tự đỏ khi B fix, lúc đó flip lại thành test thường). (2) **A circuit breaker không bao giờ open**: `CircuitBreaker.recordFailure` seed `openedAt: 0`, `isOpen` đọc `Date.now() - 0 > cooldown` = true → delete entry trước khi đạt threshold. 2 test spec skip + unskip khi A fix. (3) minor: `standingsForLeague` sort comparator có nhánh dead-code do operator precedence (GA branch không bao giờ chạy). A/B xử lý rồi G unskip/flip tests.
