...
| **C** Home + Matches UI | `agent/c-home-matches` | 2 | `src/app/(site)/page.tsx`, `src/app/(site)/matches/`, `src/app/(site)/schedule/`, `src/app/(site)/results/`, `src/components/matches/`, `src/components/charts/`, `src/components/sports/` | A (API/services), B (ui kit) | 🟢 xong (PR#6) |
...
| **H** Docker + Docs | `agent/h-docker-docs` | 3 | `Dockerfile`, `docker-compose*.yml`, `nginx/`, `docker/`, `docs/*.md`, `README.md`, `ENVIRONMENT.md`... | A–F merged | 🟢 xong |
...
[NOTE] (D) — 2026-09-02 — Follow-up (commit 2e3e341..): (1) PlayerStatsChart recharts đã thêm vào `/players/[slug]` (BarChart 5 chỉ số, theme CSS vars, responsive, tooltip dark-first) — seed hiện không có `match_events` nên chart render giá trị 0, có dữ liệu thật là hiển thị ngay. (2) Coach hiển thị ở header `/teams/[slug]` — lấy từ `match_lineups.coach_name` trận gần nhất qua `getTeamCoach()` trong wrapper (không có bảng coaches; ponytail: thay bằng `teamsService.coach()` khi A thêm). (3) Filter mùa giải cũ trên `/standings` KHÔNG làm: mỗi giải chỉ có đúng 1 season `is_current` trong seed → không có gì để filter; cần thì A thêm param season cho `leaguesService.standings` trước, rồi D mới wire UI. Typecheck + lint sạch; curl player page 200, recharts SSR OK.

[NOTE] (D) — 2026-09-02 — Follow-up 2: bổ sung theo đúng spec 4.D — (1) tab Statistics trên `/leagues/[slug]`: top 10 ghi bàn (tính từ `match_events`, link sang player page) + chart recharts "tấn công mạnh nhất" (goalsFor top 8 đội theo BXH); (2) mùa giải hiện tại hiển thị ở header league (`getCurrentSeason` → "Mùa 2025/26"); (3) squad team thêm cột bàn thắng mỗi cầu thủ (`getTeamPlayerGoals`). Seed chưa có `match_events` nên các cột hiển thị "—" cho đến khi engine/worker sinh events — query đúng, có dữ liệu là hiện. Typecheck + lint sạch; curl 4 trang 200 trên `sport_d`.

- [NOTE] (C) — 2026-09-02 — Gói C hoàn thành trên nhánh `agent/c-home-matches` (rebase sau A+B). Đã verify runtime với DB riêng `sport_c` (migrate + seed 14 live/469 matches): home (hero carousel live+breaking, LIVE NOW realtime SSE topic `home` score-bump, upcoming chips, latest results, trending news, popular leagues, ISR 60s), `/matches/[id]` (SSE `match:{id}` cập nhật score/minute/status không reload; tabs Timeline/VAR, Statistics Recharts, Lineups pitch SVG + bench + coach, H2H, Commentary), `/schedule` (sport/league/date/team-search combobox/status chips, group theo giải theo ngày), `/results` (date nav, group theo giải). typecheck + lint sạch. Đã làm: mọi section có skeleton/empty/error+retry. Chưa làm (thuộc gói khác): personalization favorites section trên home (E), og image placeholders (E). Điểm contract: hero "featured" dùng live matches + breaking news (A không có endpoint featured riêng); engine payload topic `home` chỉ `{matchId, minute}` — client invalidate + refetch, không tin payload score.

- [NOTE] (H) — 2026-09-02 — Gói H hoàn thành trên `agent/h-docker-docs`. Dockerfile multi-stage (target `runner` = standalone Next non-root + HEALTHCHECK /health; target `worker` = tsx + full node_modules cho worker/seed/migrate, non-root). docker-compose.yml: postgres16 + redis7 (healthchecks) + app + worker + nginx (ports 80; SSE location `proxy_buffering off` + read_timeout 1h; API limit_req 20r/s; static immutable 30d) + service `setup` (profile setup: migrate + seed 1 lần). docker-compose.prod.yml: 3 replicas + resource limits. Docs 7 file trong docs/ (API đầy đủ endpoint + examples, DATABASE ERD mermaid + index rationale + partition path, DEPLOYMENT single→horizontal→1M users + zero-downtime checklist, SECURITY threat checklist, TESTING, ENVIRONMENT) + README mới + .dockerignore + .env.example thêm METRICS_TOKEN/PG_POOL_MAX. Giới hạn môi trường: Docker daemon không chạy được trên máy dev này (sudo cần password) — YAML validate bằng parser, Dockerfile theo chuẩn; lần chạy `docker compose --profile setup up` đầu tiên trên máy có daemon sẽ verify cuối. Cần đảm bảo `npm run build` chạy được trên main sau merge H (đã kiểm — không đổi gì ngoài docs/docker).
