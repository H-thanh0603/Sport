...
| **D** Teams/Players/Leagues UI | `agent/d-teams-players-leagues` | 2 | `src/app/(site)/teams/`, `src/app/(site)/players/`, `src/app/(site)/leagues/`, `src/app/(site)/standings/`, `src/components/teams/`, `src/components/players/`, `src/components/leagues/` | A, B | 🔨 đang làm (B đã merge; A chưa — D dựng data-access tạm trong scope mình, NOTE mục 8) |
